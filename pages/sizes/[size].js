...
function Home({ products, collectionList, variants, size }){
  ...
  return (
    <>
      ...
      <div className="text-3xl">
        ...
        <ProductList products={products} variants={variants} category={"home-page"} size={size} ></ProductList>
...
export async function getStaticPaths(){
  const variants = await getAllProductsInCollection()
   ...
   let paths = variants[0].map(item => {

   let value = String(item.node.variants.edges[0].node.selectedOptions[0].value)
      return {
        params: { size: value }
      }
    }); 
...
export async function getStaticProps({ params }) {
  ...
  return {
    props: { products, collectionList, variants, size }, // will be passed to the page component as props
  }
}
